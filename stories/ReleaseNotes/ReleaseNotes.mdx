...
Curated product highlights for operators. This is **not** a git log and **not** a `CHANGELOG.md` — the design system does not maintain a changelog file. Current pin is **1.3.10** (auto-bumps on `main`; first LAUNCH was **1.1.0**). Technical continuity is the repo tip; product-facing continuity is this page.
...
Assessment remediation Phases 0–5 and gap closure (TASK-IMP-001…021): contrast, pack-contract, overlay manager, zoom/CQ gates, and related Storybook / template polish.

## Release — `@cyberskill/design@1.2.0`

`VERSION` **auto-bumps on `main`** (Conventional Commits → tag `v*` → Trusted Publishing). Owner may still force a bump. Curated continuity stays here — still **no** root `CHANGELOG.md`.

## Patch — `@cyberskill/design@1.1.1`

Lockstep republish after LAUNCH: natives / provenance / `_ds_bundle.js` match tip; forbidden root `CHANGELOG.md` removed.
...
Owner **LAUNCH** cut. `VERSION` and every stamp move from the pre-LAUNCH **1.0.0** pin to **1.1.0**. Publish via **npm Trusted Publishing (OIDC)** on tag `v1.1.0` (or `workflow_dispatch`). Package Publishing access on npmjs **disallows tokens**. Soft-skip on the npm-hello registry install is honest until **1.1.0** is live — soft-skip ≠ published.
...
Portable consumers: `styles.css`, `_ds_bundle.js`, bundler-native `_esm/react.mjs` (default package entry; React peer), and browser legacy `_esm/cs.mjs` (`@cyberskill/design/legacy`). Storybook is host-only tooling.
