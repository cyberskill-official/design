...
function formatCurrency(n, langOrOpts) {
  ...
  if (opts && opts.currency) {
    ...
    if (!locale) {
      const L2 = primaryLang(opts.lang) || "en";
      locale = L2 === "vi" ? "vi-VN" : L2 === "ja" ? "ja-JP" : "en-US";
    }
...
Object.assign(__ds_scope, { knownLocales, primaryLang, resolveLang, tr, makeT, useLang, formatDate, monthName, formatNumber, formatCurrency });
...
const { makeT, useLang } = __ds_scope;
const { cx } = __ds_scope;
const { EDITOR_SCHEMA, sanitizeHtml } = __ds_scope;
function Editor({ defaultValue = "", unsafeHtml, onChange, minHeight = 120, lang, className }) {
