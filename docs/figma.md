...
**What CI does today:** the `figma-variables-push` job **soft-skips (exit 0 + report) if `FIGMA_TOKEN` / `FIGMA_FILE_KEY` are empty** — same honesty as Code Connect (missing secrets are not a red main). With secrets present, file open works on any plan; the Variables write also soft-skips with a report when Figma returns **403** / invalid scopes for `file_variables`. Soft-skip ≠ live Variables sync. A green job without a matching release SHA in the report is not proof that Figma Variables or Code Connect match the published package. Bind artifacts to `VERSION` + `git rev-parse HEAD` from `scripts/release-bind.mjs`. Full auto-push needs Enterprise + those two scopes on the PAT.
