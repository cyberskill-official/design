...
## Fast board — 34 gates in `_audit/run.html`
...
| Version stamp | `_audit/version-stamp.html` | `package.json`, `tokens.json`/`tokens.js` metas, DTCG `$extensions`, `provenance.json` release + dtcgStamp, `DESIGN.md` front matter, and `_esm/cs.mjs` `export const VERSION` all equal `VERSION` (1.0.0 pin) | All 9 stamps equal (`__versionstamp`) | Hard | Fast board CI |
...
| Component behavior | `_audit/component-behavior-test.html` | 39 real interaction assertions — toggles, controlled selection, palette filtering, overlay open/close, Form rules, Tree/Splitter/keyboard, Wave-2 Item/NativeSelect/AlertDialog/Collapsible | 39/39 assertions pass (`__behavior`) | Hard | Fast board CI |
...
| React entry | `_audit/ci/test-react-entry.mjs` | Dual package exports (`.` / `./react` → `_esm/react.mjs`, `./legacy` → `_esm/cs.mjs`); VERSION pin 1.0.0; React peerDeps; react.mjs has no CDN/bundle bridge and re-exports match the bundle header; `generate-react-entry --check` green | All assertions pass (exit 0) | Hard | Unit test |
