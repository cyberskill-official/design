...
**Ship check** — chạy compiler check đến sạch, rồi chạy `_audit/docs-consistency.html` **và** `_audit/namespace-portability.html` (cả hai phải xanh). VERSION **1.6.1** (auto-bump trên `main`).
...
| Document (mọi cái có `@page{size:…}` / `<meta name="omelette-owns-print">`) | **PDF + DOCX chỉnh sửa được** | Một nút **Tải / Download** (`data-omelette-chrome`, ẩn khi in) từ `templates/_vendor/doc-export.js` qua `ds-base.js`, menu PDF \| DOCX. Cả hai format dựng từ `.cs-sheet` sống để file khớp preview HTML. **PDF** = raster sheet đã render (trang JPEG; fallback: in bản clone same-origin — không phải URL Storybook); CI vẫn dùng Playwright `page.pdf`. **DOCX** = OOXML editable walk từ cùng sheet (bảng, đoạn bilingual trong ô, heading umber, Unicode VN; header Word là dòng brand ngắn, không nhân đôi lockup); ZIP qua `CompressionStream` — không CDN. Deck vẫn PPTX. |
