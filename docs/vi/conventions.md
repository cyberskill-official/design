...
**Ship check** — chạy compiler check đến sạch, rồi chạy `_audit/docs-consistency.html` **và** `_audit/namespace-portability.html` (cả hai phải xanh). VERSION **1.5.0** (auto-bump trên `main`).
...
| Document (mọi cái có `@page{size:…}` / `<meta name="omelette-owns-print">`) | **PDF + DOCX chỉnh sửa được** | Toolbar trong iframe (`data-omelette-chrome`, ẩn khi in) từ `templates/_vendor/doc-export.js` qua `ds-base.js`. **PDF** = `window.print()` (Chromium/`@page` A4 hoặc Letter); CI dùng Playwright `page.pdf`. **DOCX** = OOXML editable walk từ `.cs-sheet` (header/footer, bảng, heading umber, Unicode VN); ZIP qua `CompressionStream` — không CDN. Deck vẫn PPTX. |
