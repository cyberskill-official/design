...
| Docs Storybook coverage | `_audit/docs-storybook-coverage.html` | Mọi doc operator tracked + entrance + doctrine có mục Storybook Docs sidebar và mục viewer; `docs/` nằm trong `staticDirs`; file entrance xuất bản tại `/` | 0 story / mục viewer thiếu (`__docssbcoverage`) | Hard | Fast board CI + unit twin |
| Docs link check | `_audit/docs-link-check.html` | Link tương đối trong markdown + MDX đã xuất bản resolve; không path đã nghỉ (`status.html`, status-legacy, Identity Lab như href sống) | 0 link trong-repo gãy (`__docslinkcheck`) | Hard | Fast board CI + unit twin |
