...
Mọi quality gate deterministic trong hệ thống, nó assert gì, tiêu chí pass, hard hay advisory, và chạy ở đâu. Xuất bản trên Storybook **Docs** tại `design.cyberskill.world`.
...
## Fast board — 42 gates trong `_audit/run.html`
...
| Docs Storybook coverage | `_audit/docs-storybook-coverage.html` | HTML sống: catalog đã xuất bản + `/docs/**` + viewer + file entrance; title `index.json` Storybook khi có (không fetch `.storybook/` / `stories/` chưa xuất bản). Node twin: nguồn CSF/MDX + `staticDirs` | 0 trang / mục viewer thiếu (`__docssbcoverage`) | Hard | Fast board CI + unit twin |
| Docs link check | `_audit/docs-link-check.html` | HTML sống: markdown đã xuất bản + link viewer resolve; không path đã nghỉ. Node twin còn quét `href`/`src` MDX chưa xuất bản | 0 link trong-repo gãy (`__docslinkcheck`) | Hard | Fast board CI + unit twin |
...
| Version stamp | `_audit/version-stamp.html` | `package.json`, meta `tokens.json`/`tokens.js`, DTCG `$extensions`, `provenance.json` release + dtcgStamp, front matter `DESIGN.md`, và `_esm/cs.mjs` `export const VERSION` đều bằng `VERSION` (auto-bump trên main) | Cả 9 stamp bằng nhau (`__versionstamp`) | Hard | Fast board CI |
...
| ESM smoke test | `_audit/esm-smoke-test.html` | `_esm/cs.mjs` import không cần build, re-export mọi component manifest, `VERSION` bằng root `VERSION`, và Button mount nhận brand token | Export/manifest parity + khớp VERSION + kiểm token (`__esm`) | Hard | Fast board CI |
...
| A11y interactions | `_audit/a11y-gate.html` | Focus trap/restore, roving, Mentions APG + Sortable buttons (hard-fail), role/label, open-state Esc | Mọi assert keyboard/ARIA pass (`__a11y`) | Hard | Fast board CI |
...
| Support runtime identity | `_audit/support-runtime-identity.html` | Mọi bản `templates/*/support.js` (một mỗi template, 85) byte-identical: sha-256 qua SubtleCrypto, đúng một hash duy nhất, số bản == số template manifest | 1 hash duy nhất, count khớp (`__supportid`) | Hard | Fast board CI |
...
| Gate | File | Assert gì | Tiêu chí pass | Loại | Chạy ở đâu |
|---|---|---|---|---|---|
| Responsive overflow | `_audit/responsive-overflow.html` | Mọi template (85) + 3 UI kit render ở 390 px không horizontal document overflow | 0 bề mặt overflow (`__overflow`) | Hard | Whole-set CI (mọi PR + nightly) |
...
| Docs Storybook coverage (Node twin) | `_audit/ci/test-docs-storybook-coverage.mjs` | Docs tracked + entrance + doctrine mỗi cái có title story Storybook và path viewer; `staticDirs` gồm `docs/` | Exit 0 | Hard | Unit test |
| Docs link check (Node twin) | `_audit/ci/test-docs-link-check.mjs` | Link tương đối trong-repo của tập xuất bản bắt buộc resolve; path đã nghỉ không là href sống | Exit 0 | Hard | Unit test |
