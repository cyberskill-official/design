...
**Ship check** — run the compiler check until clean, then run `_audit/docs-consistency.html` **and** `_audit/namespace-portability.html` (both must be green). VERSION **1.5.1** (auto-bumps on `main`).
...
| Document (any with `@page{size:…}` / `<meta name="omelette-owns-print">`) | **PDF + editable DOCX** | In-iframe toolbar (`data-omelette-chrome`, print-stripped) from `templates/_vendor/doc-export.js` via `ds-base.js`. **PDF** = `window.print()` (Chromium/`@page` A4 or Letter); CI uses Playwright `page.pdf`. **DOCX** = editable OOXML walked from `.cs-sheet` (headers/footers, tables, umber headings, VN Unicode); ZIP via `CompressionStream` — no CDN. Decks stay PPTX. |
